...
const add_order_list = (order_list) => {
  ...
  const total_html = `
  				<div class='order_list'>
					...
					<div class='order_pay_total'>${total_pay.total}</total>
				</div>`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // button이 클릭되었을 때 결제 처리를 수행하려고 했다
  // button box 포함하여 button을 동적으로 생성을 했다
  // 동적으로 생성된 tag들은 자체적으로 event를 수신하지 못한다
  // 아래의 event 핸들러는 div.button_box가 만들어지기 전에 선언되고 OS에게 알려진 코드이다
  // div.button_box가 아직 만들어지지 않은 상태에서 선언된 event 핸들러는 OS가 무시해버린다
  if (pay_box) {
  ...
  // 실제 tag가 생성된 후에 event를 버블링할 수 있도록 설정하는 방법
